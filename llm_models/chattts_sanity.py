...
def save_wav(path, wav, sr=SR_IN):
    ...
    if wav.ndim == 2:
        ...
        else: wav = wav.mean(axis=0)  # safety
    # sanitize амплитуды
...
def resample44k(mono_24k: np.ndarray) -> np.ndarray:
    ...
    if in_sr == out_sr: return mono_24k
    ratio = out_sr / in_sr
...
def try_generate(chat, text: str, seed: int, temperature: float, refine_prompt: str):
    ...
    code = ChatTTS.Chat.InferCodeParams(spk_emb=spk, temperature=temperature, top_P=0.55, top_K=20)
...
def main():
    ...
    code = ChatTTS.Chat.InferCodeParams(spk_emb=spk, temperature=0.10, top_P=0.55, top_K=20)

    # вариант А: совсем без refine
    wavs = chat.infer("Hello, my name is Anton.",
